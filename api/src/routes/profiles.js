...
router.get('/', async (req, res) => {
  try {
    const count = await db.select().from('profile').orderBy('id_profile');
...
router.post('/', async (req, res) => {
  const add = req.body;
  try {
    const count = await db.insert(add).into('profile');
    if (count !== 0) {
...
router.delete('/:id', async (req, res) => {
  const idProfile = req.params.id;
  try {
    const count = await db('profile').where('id_profile', idProfile).del();
    if (count !== 0) {
